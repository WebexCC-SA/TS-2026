...
2. Add **Custom Event** on the right-hand side of the prortal and configure it with the following: </br>
...
3. **Save** and **Publish** your changes. Enter publishing text in the popup comment window, then click **Publish**
   ![Profiles](../graphics/Lab1_AI_Agent/6.54.gif)

**<details><summary>Good to Know: <span style="color: blue;">Why we need to use a Custom Event for fulfillment.</span></summary>**

The API call to retrieve the order status can be made from the Voice Flow. By using a Custom Event, we send the **order_number** entity that the Scripted Agent collects during the call to retrieve the status.
...
3. Create 3 empty variables with type **String** and with the following names:</br>
...
5. Click on the **VirtualAgentV2** node, scroll down to Activity Output Variables, and copy the MetaData variable name. Then, open the **SetVariable** node and configure the **MetaData_Scripted_Agent** variable with the variable value you copied from the VirtualAgentV2 node. Make sure to enclose it in curly brackets **{ }**.<br> 
Ex. ***{{VirtualAgentV2_i25_p40.MetaData}}***.
    ![Profiles](../graphics/Lab1_AI_Agent/6.47.gif)

6. Add **Parse** node and connect it to **SetVariable** node.
    ![Profiles](../graphics/Lab1_AI_Agent/6.48.gif)

7. Configure the **Parse** node with the following:</br>
...
9. Add an **HTTPRequest** node and connect it to **parse** node.
    ![Profiles](../graphics/Lab1_AI_Agent/6.51.gif)

10. Configure the **HTTP Request** node with the following:<br>

    > Use Authenticated Endpoint: **off**<br>
    > Request URL: **https://674481b1b4e2e04abea27c6e.mockapi.io/flowdesigner/Lab/flowers**<span class="copy-static" data-copy-text="https://674481b1b4e2e04abea27c6e.mockapi.io/flowdesigner/Lab/flowers"><span class="copy" title="Click to copy!"></span></span><br>
    > Method: **GET**<br>
    > Queue Parameters: <br>
...
11. Add **Disconnect Contact** and connect **HTTPRequest** node to the **Disconnect Contact** node. **Validate** and **Publish** the flow.
